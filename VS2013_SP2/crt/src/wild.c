...
struct argnodeW {
    wchar_t *argptr;
    struct argnodeW *nextnode;
};

static struct argnodeW *arghead;
static struct argnodeW *argend;

static int __cdecl wmatch(wchar_t *, wchar_t *);
static int __cdecl wadd(wchar_t *);
static void __cdecl wsort(struct argnodeW *);
static wchar_t * __cdecl wfind (wchar_t *pattern);
#else  /* WPRFLAG */
struct argnodeA {
    char *argptr;
    struct argnodeA *nextnode;
};

static struct argnodeA *arghead;
static struct argnodeA *argend;

static int __cdecl match(char *, char *);
static int __cdecl add(char *);
static void __cdecl sort(struct argnodeA *);
        ...
        )
{
        ...
        struct argnodeW *nodeptr;
#else  /* WPRFLAG */
        char **argv = __argv;
        struct argnodeA *nodeptr;
#endif  /* WPRFLAG */
        int argc;
        _TSCHAR **tmp;
        _TSCHAR *wchar;
        ...
        )
{
        ...
        _TSCHAR *all;
#ifdef WPRFLAG
        struct argnodeW *first;
#else  /* WPRFLAG */
        struct argnodeA *first;
#endif  /* WPRFLAG */
        int gotone = 0;
    ...
    )
{
        ...
        if (!(nodeptr = (struct argnodeW *)_malloc_crt(sizeof(struct argnodeW))))
            return(-1);
#else  /* WPRFLAG */
        struct argnodeA *nodeptr;

        if (!(nodeptr = (struct argnodeA *)_malloc_crt(sizeof(struct argnodeA))))
            return(-1);
#endif  /* WPRFLAG */
